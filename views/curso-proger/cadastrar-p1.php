...
$this->title = (isset($nome))?'Curso '.$nome: 'Novo curso';
?>

<h1><?=(isset($nome))?'Curso '.$nome: 'Novo curso'?></h1>
<?= fluxo::widget(['index'=>1, 'novo'=>$novo, 'action'=>'curso-proger/cadastrar', 'idmodel'=>$model->id]); ?>

<div class="well">
    <?php $form = ActiveForm::begin(); ?>
    <?= $form->field($model, 'nome')->textInput(['maxlength' =>300,'style' =>'width: 50%' ]) ?>

    <?= $form->field($model, 'idTipoCurso')->dropDownList(ArrayHelper::map(TipoCurso::find()->orderBy('nome')->where(['ativo' => 1])->all(),'id', 'nome'),['prompt'=>'Selecione um tipo de curso', 'style' =>'width: 50%']) ?>

    <?= $form->field($model, 'idTipoEncargo')->dropDownList(ArrayHelper::map(TipoEncargo::find()->orderBy('nome')->all(),'id', 'nome'),['prompt'=>'Selecione a natureza do curso', 'style' =>'width: 50%']) ?>

    <?= $form->field($model, 'descricao')->textarea(['maxlength' =>500,'style' =>'width: 50%' ])?>

    <?php // Se nome do curso existe, exibe campo Situação para poder ser alterado
    if($nome == true){
    ...
    <?= $form->field($model, 'documentos')->textarea(['maxlength' =>500,'style' =>'width: 50%' ]) ?> 
    ...
    <?php
        $tipoProgerCurso = ArrayHelper::map(TipoProger::find()->andwhere(['nome'=>['Programa','Projeto']])->all(), 'id', 'nome');
    ...
    <?php
        ...
        switch ($tipo) {
            case 'Programa':
                $progers = ArrayHelper::map(ProgramaProger::find()->where(['idGestor'=> $model->idGestor])->all(), 'id', 'nome');
                break;
            case 'Projeto':
                $progers = ArrayHelper::map(ProjetoProger::find()->where(['idGestor'=> $model->idGestor])->all(), 'id', 'nome');
                break;
            case 'Curso':
                $progers = ArrayHelper::map(CursoProger::find()->where(['idGestor'=> $model->idGestor])->all(), 'id', 'nome');
                break;
            case 'Evento':
                $progers = ArrayHelper::map(EventoProger::find()->where(['idGestor'=> $model->idGestor])->all(), 'id', 'nome');
                break;
            default:
                break;
        }

        echo $form->field($model, 'idProger')->dropDownList( $progers, ['prompt'=>'Selecione um proger', 'style' =>'width: 50%']);
    ...
    <?=$model->isNewRecord ?'': Html::a('Avançar (Etapa 2)', ['/curso-proger/cadastrar','n'=>$novo, 's'=>2,'idmodel'=>$model->id],['class'=>'btn btn-warning','title'=>"Esta ação não salva dados, certifique-se de salvar os dados antes de prosseguir"]) ?>

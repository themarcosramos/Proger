...
<div class="integrante-form">
    ...
    <?= (!$update)?$form->field($pessoa, 'cpf', $options = ['labelOptions'=>['label'=>'Consultar CPF']])->textInput(['maxlength' =>14,'onBlur'=>'js:buscarPessoa();','onkeypress'=>'js:mascara(this,cpf);','style' =>'width: '.$fieldWidth,'disabled'=>true,]): $form->field($pessoa, 'cpf')->textInput(['disabled'=>true, 'style' =>'width: '.$fieldWidth])?>
    ...
    <?= $form->field($model, 'idTipoVinculo')->dropDownList(ArrayHelper::map(TipoVinculo::find()->orderBy('descricao')->where(['ativo' => 1])->all(),'id', 'descricao'),['prompt'=>'Selecione um vínculo', 'style' =>'width: '.$fieldWidth,'disabled'=> true]) ?>

    <?=$form->field($model, 'idTipoFuncao')->dropDownList([$idBolsista => 'Bolsista'] + ArrayHelper::map(TipoFuncao::find()->orderBy('descricao')->where(['ativo' => 1])->all(),'id', 'descricao'),['prompt'=>'Selecione uma função','style' =>'width: '.$fieldWidth, 'id'=>'options', 'onchange'=>'js:optionCheck(),','disabled'=> true]);?>
    
    <div id="curso" style="<?php if($model->idCurso == true){ echo 'display:block';}else{ echo 'display:none';}?>">
    <?=$form->field($model, 'idCurso')->dropDownList(ArrayHelper::map(Curso::find()->orderBy('nome')->where(['ativo' => 1])->all(),'id', 'nome'),['prompt'=>'Selecione um curso','disabled'=>true, 'style' =>'width: '.$fieldWidth,'id'=>'campo-curso'])?>
    </div>

    <?= $form->field($model, 'idInstituicao')->dropDownList(ArrayHelper::map(Instituicao::find()->orderBy('instituicao')->where(['ativo' => 1])->all(),'id', 'instituicao'),['prompt'=>'Selecione uma instituição', 'style' =>'width: '.$fieldWidth,'disabled'=> true]) ?>

    <?= $form->field($model, 'idSetor')->dropDownList(ArrayHelper::map(Setor::find()->orderBy('nome')->where(['ativo' => 1])->all(),'id', 'nome'),['prompt'=>'Selecione um setor', 'style' =>'width: '.$fieldWidth,'disabled'=> true]) ?>
    ...
    <?= $form->field($model, 'matricula')->textInput(['maxlength' =>15,'style' =>'width: '.$fieldWidth, 'placeholder'=>'Apenas números','disabled'=>true]) ?>
   
    <?= $form->field($model, 'cargaHoraria')->textInput(['maxlength' =>4,'style' =>'width: '.$fieldWidth, 'placeholder'=>'Apenas números','disabled'=>true]) ?>

...
<?= Fluxo::widget(['index'=>2, 'novo'=>$novo, 'action'=>'projeto-proger/cadastrar', 'idmodel'=>$idmodel]); ?>

<!-- Se formType == 1, renderiza o gridview de integrantes relaconado ao projeto, caso contrario nao renderiza nada -->
<?= !$formType? IntegranteWidget::widget(['type'=>'gridview','controller'=>'projeto-proger','idmodel'=>$idmodel, 'idTipoProger'=>$integrante->idTipoProger, 'novo'=>$novo]):''?>
...
<div class="well"> 
    ...
    <?php 
        if($formType)
            ...
            echo IntegranteWidget::widget(['integrante'=>$integrante, 'controller'=>'projeto-proger', 'updateIntegrante'=>$updateIntegrante, 'cpf'=>$cpf,'idmodel'=>$idmodel, 'novo'=>$novo]); 
    ?>
    <!-- Botoes de voltar e avancar so aparecem se o formType for 0 -->
    
    <div class="form-group" style="display:<?= $formType? 'none': 'block'?>">           
         <a href="?r=projeto-proger/cadastrar&n=<?= $novo?>&s=1&idmodel=<?php echo $idmodel?>"><button class="btn btn-warning">Voltar(Etapa 1)</button></a>
  
        
         <?=(\app\models\Integrante::find()->where(['idProger'=>$idmodel, 'idTipoProger'=>$integrante->idTipoProger])->all()!=null)? Html::a(Html::button ('Avançar (Etapa 3)', ['class' => 'btn btn-warning','title'=>"Esta ação não salva dados, certifique-se de salvar os dados antes de prosseguir"]), ['/projeto-proger/cadastrar','n'=>$novo, 's'=>3,'idmodel'=>$idmodel]):''?>

    </div>
    <!-- Botão cancelar só surge se o formType = 1 -->
    <div class="form-group" style="display:<?= $formType? 'block': 'none'?>">  
        <a href="?r=projeto-proger/cadastrar&n=<?= $novo?>&s=2&idmodel=<?php echo $idmodel?>"><button class="btn btn-warning">Cancelar</button></a>
    </div>
</div>
<div>    

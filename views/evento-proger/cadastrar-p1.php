...
<?= Fluxo::widget(['index'=>1, 'novo'=>$novo, 'action'=>'evento-proger/cadastrar', 'idmodel'=>$model->id]);?>
...
<div class="well">
    ...
    <?= $form->field($model, 'idTipoEncargo')->dropDownList(ArrayHelper::map(TipoEncargo::find()->orderBy('nome')->all(),'id', 'nome'),['prompt'=>'Selecione a natureza do evento', 'style' =>'width: 50%']) ?>
    ...
    <?= $form->field($model, 'idTipoEvento')->dropDownList(ArrayHelper::map(TipoEvento::find()->orderBy('nome')->where(['ativo' => 1])->all(),'id', 'nome'),['prompt'=>'Selecione um tipo de evento', 'style' =>'width: 50%'])  ?>    
    ...
    <?= $form->field($model,'numeroParticipantes')->textInput(['maxlength'=>8,'style'=>'width: 30%','onlynumber' => 'true']) ?>    
...
<div class="form-group">
    ...
    <?=$model->isNewRecord ?'': Html::a('Avançar (Etapa 2)', ['/evento-proger/cadastrar','n'=>$novo, 's'=>2,'idmodel'=>$model->id],['class'=>'btn btn-warning','title'=>"Esta ação não salva dados, certifique-se de salvar os dados antes de prosseguir"]) ?>
